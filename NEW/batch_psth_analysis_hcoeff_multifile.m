...
for k = 1:numel(filePairs)
    ...
    % Convert stim times to seconds and ensure sorted unique values
    stimTimes = stimTimes_ms / 1000;
    stimTimes = sort(unique(stimTimes(:)));
    ...
    if ~isempty(networkResponse)
        output_filename = fullfile(outputDir, sprintf('networkResponse_all_channels_%s.mat', baseName));
